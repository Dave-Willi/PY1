...
class Config_File:
	...
	# saves file
	def save(self, backup_directory: str):
		...
		# checks directory exists bit by bit
		for directory in backup_directory.split("/"):
			# adds the next layer of directory
			current_dir_to_check += f"{directory}/"
			# true if theres a folder already there
			folder_exists = os.path.exists(current_dir_to_check)
			# if backup folder dosen't exist
			if (not folder_exists):
				# create the directory
				os.mkdir(current_dir_to_check)

		# moves current file into back up folder
		shutil.move(self.file_path, backup_directory + filename)
	...
	def get_section_items(self, section: str) -> dict:
		...
		# if section does NOT exist return empty dictionary
		else:
			return {}
	...
	# allows you to add a section returns false if an error occurs
	...
	# allows you to add an field to a section
