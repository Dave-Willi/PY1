...
# TODO remove dependancys on edit_frame using events
# TODO add labels above combobox and listbox
...
class Select_Frame(Custom_Window):
	...
	def __init__(self, master, file_manager: File_Manager, edit_frame: Edit_Frame, *args, **kwargs):
		Custom_Window.__init__(self, master, file_manager, *args, **kwargs)

		# create reference to the edit freame
		self.edit_frame = edit_frame
		# create label for the dropdwn box and listbox
		dropdown_box_label = tk.Label(self, text="Select a file:")
		...
		sections_listbox_label = tk.Label(self, text="Select a section:")
		# create the section selection box (listbox)
		self.sections_selection_list = Sections_Listbox(main_frame, file_manager, self.edit_frame, selectmode=tk.SINGLE)
		# create scroll bar
		...
		# render widgets
		dropdown_box_label.pack()
		self.select_file_dropdown_menu.pack()

		sections_listbox_label.pack()
	...
	def on_file_changed(self, event):
		...
		# resets selection
		self.file_manager.currently_selected_section = None
		# refreshes the listbox
		self.sections_selection_list.refresh_listbox_entrys()
		# refreshes the edit fields secion
		self.edit_frame.refresh_field_editors()
...
# the listbox on the left side containing all the sections
class Sections_Listbox(tk.Listbox):
	...
	def get_listbox_selected_name(self) -> str:
		...
		# true if there is a section selected
		section_is_selected = index is not None
		# if a section is selected ...
		if (section_is_selected):
			# gets name of selecten file
			section_name = self.get(index)
		# if there is NOT a section selected ...
		else: 
			section_name = None
